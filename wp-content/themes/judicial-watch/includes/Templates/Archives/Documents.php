...
class Documents extends AbstractTemplate {
    ...
    {
        if ('document_categories' === $this->queriedObject->taxonomy) {
            return $this->queriedObject->slug;
        }
    }

    public function getSelectedTag()
    {
        if ('document_tags' === $this->queriedObject->taxonomy) {
            return $this->queriedObject->slug;
        }
    }
    ...
    public function getFilterCategories()
    {
        ...
        $categories = collect($wpTerms)->transform(function(WP_Term $wpTerm) {
            return (object)[
                'slug'  => $wpTerm->slug,
                'title' => $wpTerm->name,
                'link'  => get_term_link($wpTerm)
            ];
        });
    ...
    public function getFilterTags()
    {
        $wpTerms = get_terms([
            ...
            'order'    => 'DESC',
            'number'   => 20,
            'taxonomy' => 'document_tags',
            'post_type' => 'documents'
        ]);

        $tags = collect($wpTerms)->transform(function(WP_Term $wpTerm) {
            return (object)[
                'slug'  => $wpTerm->slug,
                'title' => $wpTerm->name,
                'link'  => get_term_link($wpTerm)
	...
	public function getSearchParams()
    {
        $searchParams = [
            's'      => get_query_var('s'),
            'month'  => data_get($_GET, 'month'),
            'year'   => data_get($_GET, 'year'),
        ];
