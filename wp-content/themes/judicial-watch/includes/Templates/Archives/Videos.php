...
class Videos extends AbstractTemplate {
	...
	protected function addToThisPageContext() {
        ...
        $pageBackgroundImage = get_theme_file_uri('assets/images/body-bg-jwtv.jpg');
        $termId = data_get($this->queriedObject, 'term_id');
        if ($termId) {
            $acfBackgroundImage = get_field('background_image', $this->queriedObject);
            $acfBackgroundImage = isset($acfBackgroundImage['url']) ? $acfBackgroundImage['url'] : null;

            if ($acfBackgroundImage) {
                $pageBackgroundImage = $acfBackgroundImage;
            }
        }
		...
		data_set($this->context, 'backgroundImage', $pageBackgroundImage);
	}
